 ...
 */
export function readOverride(transcriptPath: string, homeDir: string = os.homedir()): OverrideFile | null {
  let raw: string;
  try {
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<OverrideFile>;
    if (!parsed || typeof parsed !== 'object') return null;
    if (parsed.version !== 1) return null;
    if (typeof parsed.transcriptPath !== 'string') return null;
    if (typeof parsed.writtenAt !== 'string') return null;
    if (parsed.clearAgentsBefore != null && typeof parsed.clearAgentsBefore !== 'string') return null;
    if (parsed.clearTodosBefore != null && typeof parsed.clearTodosBefore !== 'string') return null;
    return {
 ...
 */
export function applyOverrides(data: TranscriptData, override: OverrideFile | null): TranscriptData {
  ...
  const overridesApplied: NonNullable<TranscriptData['overridesApplied']> = {};
