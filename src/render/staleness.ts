 ...
 */
function hasNewerToolActivity(start: Date, transcript: TranscriptData): boolean {
  const t = start.getTime();
  for (const tool of transcript.tools) {
    if (tool.startTime.getTime() >= t) return true;
    if (tool.endTime && tool.endTime.getTime() >= t) return true;
  }
  return false;
}
...
export function isAgentStale(
  ...
  const idle = transcript.transcriptMtimeMs != null
    ...
    : 0;
  if (!(agentAge >= s.agentMs && idle >= s.sessionIdleMs)) return false;
  if (hasNewerToolActivity(agent.startTime, transcript)) return false;
  return true;
}
...
export function isTodoStale(
  ...
  const idle = transcript.transcriptMtimeMs != null
    ? now - transcript.transcriptMtimeMs
    : 0;
  if (!(todoAge >= s.todoMs && idle >= s.sessionIdleMs)) return false;
  if (hasNewerToolActivity(todo.startTime, transcript)) return false;
  return true;
}
