...
import { readOverride, type OverrideFile } from './overrides.js';
 ...
 */
export function parseClearArgs(argv: readonly string[]): ParseResult {
  let clearRaw: string | undefined;
  ...
  const targets = new Set<'agents' | 'todos'>();
 ...
 */
export function runClear(args: ClearArgs, deps: RunClearDeps = {}): number {
  ...
  try {
    ...
    console.error(`claude-hud: cannot create override directory: ${(e as Error).message}`);
    return 1;
  }

  const existing: OverrideFile | null = readOverride(transcriptPath, homeDir);

  const monotonic = (incomingMs: number, prior: string | undefined): string => {
  ...
  try {
    ...
    console.error(`claude-hud: override write failed: ${(e as Error).message}`);
    return 1;
  }
  ...
  try {
    ...
    const verify = JSON.parse(verifyRaw) as Partial<OverrideFile>;
    if (!verify || typeof verify !== 'object') throw new Error('parsed value not an object');
    if (verify.version !== 1) throw new Error('schema mismatch');
    if (typeof verify.transcriptPath !== 'string') throw new Error('missing transcriptPath');
    if (typeof verify.writtenAt !== 'string') throw new Error('missing writtenAt');
    ...
    console.error(`claude-hud: override verify failed: ${(e as Error).message}`);
    return 1;
  }
