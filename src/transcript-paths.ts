 ...
 */
export function getTranscriptHash(transcriptPath: string): string {
  return createHash('sha256').update(path.resolve(transcriptPath)).digest('hex');
}

/** Cache file path for a parsed transcript. */
export function getTranscriptCachePath(transcriptPath: string, homeDir: string): string {
...
export function getOverrideFilePath(transcriptPath: string, homeDir: string): string {
