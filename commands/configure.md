...
### Q: Agents Format (only if Agents is or will be enabled)
- header: "Agents Format"
- question: "How should running agents be shown?"
- multiSelect: false
- options:
  - "Compact (default)" - Single line: ◐ explore [haiku]: desc (2m 15s)
  - "Multi-line (tree)" - One line per agent with tree characters: ├─ e explore  45s  desc
...
| Agents format | `display.agentsFormat` |
| Agents max lines | `display.agentsMaxLines` |
...
6. Apply Agents Format answer. If Agents ends up OFF after the flow, ignore the Agents Format answer entirely.
...
8. Apply Agents Format answer. If Agents ends up OFF after the flow, ignore the Agents Format answer entirely.

**Guided flow scope note:** `display.agentsMaxLines` is NOT asked in the guided flow (default 5) but is preserved on save and can be edited manually in `config.json`.
